...
//  YZCustomButton
//
//  Created by ice on 07/27/2021.
//  Copyright (c) 2021 ice. All rights reserved.
//
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0 ..< titleList.count {
            ...
            button.frame = CGRect(x: 10, y: 40 + i * 44, width: 120, height: 44)
            button.addTarget(self, action: #selector(buttonClick(sender:)) , for: .touchUpInside)
            
            if i == titleList.count - 1 {
                scrollView.contentSize = CGSize(width: self.view.frame.size.width, height: button.frame.origin.y + 100)
            }
        }
